...
import { useSurplusAllocationSWR } from "@/hooks/swr/use-surplus-allocation";
...
export function SurplusAllocationMessage({ 
    ...
    return (
      <div className="bg-white rounded-lg p-6 text-center max-w-lg">
        ...
        <Button 
          ...
          onClick={() => onSendMessage && onSendMessage("I want to create an emergency fund goal")}
        >
  ...
  const handleConfirmAllocation = async () => {
    ...
    if (success && onSendMessage) {
      onSendMessage(
        `I've allocated ${formatVND(allocationAmount)} from my surplus to my emergency fund!`
      );
    }
  };

  const sliderPercentage = data.surplusAmount > 0 
    ? (allocationAmount / data.surplusAmount) * 100 
    : 0;
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-green-600">
              {t("surplusSavedAmount", { amount: formatVND(data.surplusAmount) })}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600">{t("surplusAllocationAmount")}</span>
      ...
      {allocationAmount > 0 && (
        <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-blue-700">{t("surplusMonthsCoverage")}</span>
              <span className="font-medium text-blue-900">
                {t("surplusMonthsCount", { count: Math.round(data.newMonthsOfCoverage * 10) / 10 })}
              </span>
            </div>
            
            {data.progressImpact > 0 && (
              <div className="flex justify-between">
                <span className="text-blue-700">{t("surplusProgressGain")}</span>
      ...
      <div className="space-y-3 pt-4 border-t border-gray-100">
        {allocationAmount > 0 ? (
          ...
          >
            ...
            {isAllocating 
              ? t("surplusAllocating") 
              : t("surplusConfirmAllocation", { amount: formatVND(allocationAmount) })
            }
          </Button>
        ) : (
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button 
            variant="outline" 
            size="sm"
            onClick={() => onSendMessage && onSendMessage("Show me other ways to use my surplus money")}
          >
          ...
          <Button 
            variant="outline" 
            size="sm"
            onClick={() => onSendMessage && onSendMessage("Help me set savings goals for next month")}
          >
