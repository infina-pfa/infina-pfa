...
import { TrendingUp, Target, Calendar, Plus, Wallet, Clock } from "lucide-react";
...
export function GoalDashboardMessage({ onSendMessage }: GoalDashboardMessageProps) {
  ...
  const { data, loading, error } = useGoalDashboardSWR();
    ...
    <div className="bg-white p-6 space-y-6" style={{ fontFamily: 'Nunito, sans-serif' }}>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center justify-center">
            <Wallet className="h-5 w-5 mr-2" style={{ color: '#0055FF' }} />
            <span className="text-sm font-medium text-gray-700">{t("currentAmount")}</span>
          </div>
        ...
        >
          <div className="flex items-center justify-center">
            <Target className="h-5 w-5 mr-2" style={{ color: '#0055FF' }} />
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center text-gray-700">
              <TrendingUp className="h-4 w-4 mr-2" style={{ color: '#2ECC71' }} />
              <span className="text-sm font-medium">{t("goalDashboardMonthsCoverage")}</span>
            </div>
            <span 
              className="font-bold text-base"
              style={{ color: '#2ECC71' }}
            >
              {t("goalDashboardMonthsCount", { count: data.monthsOfCoverage })}
            </span>
          </div>
      ...
      <div className="pt-2">
        <Button 
          ...
          style={{ backgroundColor: '#0055FF', fontFamily: 'Nunito, sans-serif' }}
          onClick={() => onSendMessage && onSendMessage("I want to add money to my emergency fund")}
        >
          <Plus className="h-5 w-5 mr-2" />
