  ...
  const { data, error, isLoading, mutate } = useSWR(
    ...
    async () => {
      try {
        ...
        } else if (financialOverview?.overview?.totalIncome && financialOverview?.overview?.totalExpense) {
          // Calculate based on user's actual income and expenses
          const monthlyDisposableIncome = financialOverview.overview.totalIncome - financialOverview.overview.totalExpense;
          const conservativeSavingsRate = Math.max(0, monthlyDisposableIncome * 0.2); // 20% of disposable income
          
          if (conservativeSavingsRate > 0 && remainingAmount > 0) {
            monthlySavingsRate = conservativeSavingsRate;
            estimatedMonthsToComplete = Math.ceil(remainingAmount / conservativeSavingsRate);
          } else {
            // Fallback to a conservative estimate
            monthlySavingsRate = Math.min(500000, targetAmount * 0.05); // 5% of target or 500k VND
            estimatedMonthsToComplete = monthlySavingsRate > 0 ? Math.ceil(remainingAmount / monthlySavingsRate) : 36;
          }
        } else {
        ...
        projectedDate.setMonth(projectedDate.getMonth() + estimatedMonthsToComplete);
        const projectedCompletionDate = remainingAmount > 0 ? projectedDate.toISOString() : "";
