...
export interface UseRecentTransactionsSWRReturn {
  ...
  isLoading: boolean;
  error: Error | null;
  mutate: () => Promise<Array<Transaction & { 
    budgetName?: string; 
    budgetColor?: string;
  }> | undefined>;
}

export const useRecentTransactionsSWR = (months: number = 6): UseRecentTransactionsSWRReturn => {
  ...
  const { 
    ...
    async () => {
      const response = await transactionService.getRecentTransactions(months, t);
      if (response.error) {
