...
import type { UserUpdate, UserProfile, BudgetingStyle } from "@/lib/types/user.types";

interface UseUserUpdateSWRReturn {
  updateUserProfile: (updates: UserUpdate) => Promise<UserProfile | null>;
  updateBudgetingStyle: (budgetingStyle: BudgetingStyle) => Promise<UserProfile | null>;
  isUpdating: boolean;
  error: string | null;
}
...
export function useUserUpdateSWR({
  ...
  const updateUserProfile = async (updates: UserUpdate): Promise<UserProfile | null> => {
    try {
      setIsUpdating(true);
      setError(null);
  ...
  const updateBudgetingStyle = async (budgetingStyle: BudgetingStyle): Promise<UserProfile | null> => {
    try {
      setIsUpdating(true);
      setError(null);

      // Call existing service
      const response = await userService.updateBudgetingStyle(budgetingStyle, t);
