...
export function useMonthlyBudgetAnalysisSWR(): UseMonthlyBudgetAnalysisSWRReturn {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    ...
    async () => {
      ...
      const transactionsResponse = await transactionService.getRecentTransactions(1, t);
      ...
      const overspentCategories = currentBudgets
        ...
        .map(budget => ({
          ...
          percentage: budget.amount > 0 ? ((budget.spent - budget.amount) / budget.amount) * 100 : 0,
