  ...
  const { data, error, isLoading, mutate } = useSWR(
    ...
    async () => {
      ...
      if (emergencyFundGoal.due_date) {
        ...
        const monthsDiff = (dueDate.getFullYear() - currentDate.getFullYear()) * 12 + 
                          (dueDate.getMonth() - currentDate.getMonth());
        monthsToComplete = Math.max(1, monthsDiff);
      }

      // Calculate required monthly amount to reach goal
      const monthlyRequiredAmount = remainingAmount > 0 ? remainingAmount / monthsToComplete : 0;
      ...
      if (remainingAmount <= 0) {
        ...
        const adjustedMonths = Math.ceil(remainingAmount / userFinancialCapacity);
      ...
      const maxRecommendedAmount = Math.round(Math.min(recommendedAmount * 2, userFinancialCapacity * 1.5));
