...
export function useSurplusAllocationSWR(): UseSurplusAllocationSWRReturn {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    ...
    async () => {
      ...
      const estimatedMonthlyExpenses =
        totalSpent > 0 ? totalSpent : totalBudget / 3;
      const newMonthsOfCoverage =
  ...
  const updatedData = data
    ? {
        ...
        newMonthsOfCoverage:
          data.currentEmergencyFund + allocationAmount > 0
            ? (data.currentEmergencyFund + allocationAmount) /
              (data.surplusAmount || 1)
            : 0,
        progressImpact: data.emergencyFundGoal?.targetAmount
          ? ((data.currentEmergencyFund + allocationAmount) /
              data.emergencyFundGoal.targetAmount) *
              100 -
            (data.currentEmergencyFund / data.emergencyFundGoal.targetAmount) *
              100
          : 0,
      }
    : null;
