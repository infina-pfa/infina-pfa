...
class ChatMessageManagerService {
  ...
  async saveChatMessage(
    ...
    console.log(`💾 Saving ${sender} message (order: ${order}, critical: ${critical})`);
  ...
  private async saveSynchronously(
    ...
    try {
      ...
      console.error(`❌ Failed to save ${sender} message synchronously:`, error);
      throw error;
    }
  }
  ...
  private async saveAsynchronously(
    ...
    try {
      ...
      savePromise
        .then(() => {
          this.pendingSaves.delete(messageId);
          console.log(`✅ Asynchronously saved ${sender} message (order: ${order})`);
        })
        .catch((error) => {
          this.pendingSaves.delete(messageId);
          console.error(`❌ Failed to save ${sender} message asynchronously:`, error);
        });
