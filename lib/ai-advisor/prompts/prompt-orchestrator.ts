...
export type FinancialStage = 'debt' | 'start_saving' | 'start_investing' | 'none';
...
export function generateStageSpecificPrompt(
  ...
  if (!identifiedStage || identifiedStage === 'none') {
    ...
    return generateOnboardingSystemPrompt(userId, userProfile, conversationHistory, currentStep);
  }
  ...
  switch (identifiedStage) {
    case 'debt':
      ...
      return generateDebtStagePrompt(userId, userProfile, conversationHistory, currentStep);
      
    case 'start_saving':
      console.log('💰 Using START SAVING stage prompt');
      return generateStartSavingStagePrompt(userId, userProfile, conversationHistory, currentStep);
      
    case 'start_investing':
      console.log('📈 Using START INVESTING stage prompt');
      return generateStartInvestingStagePrompt(userId, userProfile, conversationHistory, currentStep);
      
    default:
      console.warn('⚠️ Unknown stage, falling back to onboarding prompt:', identifiedStage);
      return generateOnboardingSystemPrompt(userId, userProfile, conversationHistory, currentStep);
  }
}
    ...
    case 'start_investing':
      if (hasHighInterestDebt) {
        return {
          isValid: false,
          recommendedStage: 'debt',
          reason: 'User has high-interest debt that should be addressed before investing'
        };
      }
      if (!hasEmergencyFund || (emergencyFundMonths && emergencyFundMonths < 3)) {
        return {
          isValid: false,
      ...
      if (!hasHighInterestDebt) {
        if (!hasEmergencyFund) {
          ...
          return {
            ...
            reason: 'User has cleared debt and has emergency fund - ready for investing'
          };
        }
      }
      return { isValid: true };
  ...
  switch (currentStage) {
    case 'debt':
      return {
        ...
        description: 'Eliminate high-interest debt to build financial foundation',
        completionCriteria: [
