...
export async function GET() {
  try {
    ...
    const { data: goals, error: goalsError } = await supabase
      ...
      .eq("user_id", user.id)
      .or(
        "title.ilike.%emergency%,title.ilike.%dự phòng%,title.ilike.%khẩn cấp%,title.ilike.%fund%"
      )
    ...
    if (!emergencyFundGoal) {
      ...
      emergencyFundGoal = allGoals?.[0] as Goal;
    }
    ...
    if (budgets && budgets.length > 0) {
      budgets.forEach((budget) => {
        if (budget.budget_transactions) {
          budget.budget_transactions.forEach(
            (bt: {
    ...
    if (totalMonthlyExpenses === 0 && budgets && budgets.length > 0) {
      totalMonthlyExpenses = budgets.reduce(
    ...
    const targetAmount =
      emergencyFundGoal.target_amount || totalMonthlyExpenses * 3; // 3 months of expenses
    ...
    const monthsOfCoverage =
      totalMonthlyExpenses > 0 ? currentAmount / totalMonthlyExpenses : 0;
